...
<div class="relative">
    <img
        src="{{ asset($product->image ?? 'images/cup.png') }}"
...
<div class="text-sm text-gray-700 mb-4 flex gap-2">
    @php
        $image = asset('images/' . ($product->image ?? 'cup.png'));
    @endphp
...
<!-- Modal -->
    <div id="gcashModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white p-6 rounded shadow-md max-w-sm w-full">
            <h2 class="text-lg font-semibold mb-4">Confirm Payment via GCash</h2>
            <p class="mb-4">Are you sure you want to proceed to PayMongo to pay via GCash?</p>
            <div class="flex justify-end space-x-2">
                <button onclick="closeGcashModal()" class="bg-gray-300 px-4 py-2 rounded">Cancel</button>
                <button onclick="confirmGcash()" class="bg-gray-300 px-4 py-2 rounded">Yes,Confirm</button>
...
<div class="mt-6">
    ...
    <div class="flex justify-between mt-2">
        ...
        <button
            ...
            onclick="selectPayment(this); openGcashModal();">
            Cash
        </button>
    </div>
</div>
...
<form action="{{ route('orders.store') }}" method="POST" id="orderForm">
    ...
    <button type="submit" class="mt-4 w-full bg-green-600 text-white py-2 rounded" id="placeOrderBtn">Place Order</button>
</form>
...
window.addToBillingCart = function (productName, option, price, image = 'images/cup.png') {
    const cartItemHTML = `
        <div class="bg-white p-3 rounded shadow-md cart-item" data-price="${price}">
            <div class="flex justify-between items-start gap-4">
                ...
                <div class="flex-1">
                    <div class="flex justify-between">
                        <div>
                            <h3 class="font-semibold">${productName} <span class="text-sm text-gray-500">(${option})</span></h3>
                            <p class="text-sm">₱ <span class="item-price">${price}</span></p>
                        </div>
                        <button onclick="removeCartItem(this)">
                            <img src="/images/close.png" alt="Remove Item" class="w-5 h-5">
                        </button>
                    </div>
...
function confirmGcash() {
    console.log("Confirm GCash clicked");
    ...
    const totalAmount = Math.round(parseFloat(totalAmountText.replace("₱", "")) * 100); // convert to centavos
    ...
    fetch("/paymongo/gcash", {
        ...
        body: JSON.stringify({
            items: cartItems,
            payment_method: "gcash", // lowercase is cleaner and consistent
            total_amount: totalAmount
        })
    })
    .then(res => res.json())
.then(data => {
    console.log("PayMongo Response:", data); // Add this!
...
document.getElementById("placeOrderBtn")?.addEventListener("click", function (event) {
        ...
        // Make sure cartItems are defined (from your existing logic)
        document.getElementById("itemsInput").value = JSON.stringify(cartItems);
        document.getElementById("totalAmountInput").value = document.getElementById("totalAmount").textContent.replace("₱", "").trim();

        // IF CASH, do localStorage + redirect
        ...
        // IF GCASH, let the form submit to Laravel and Laravel will handle redirect
    });
